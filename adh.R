...
data_rhoa-Fact-d <- read.csv('rhoa-Fact-d.csv', sep = ';')

ggplot(data_rhoa-Fact-d, aes(x = probe, y = bTau)) +
...
fit_rhoa-Fact-d <- lm(bTau ~ probe, data = data_rhoa-Fact-d)

summary(fit_rhoa-Fact-d)

###############################
#4# RhoA/F-actin colocalization 
data_rhoa-Nuc-d <- read.csv('rhoa-Nuc-d.csv', sep = ';')

ggplot(data_rhoa-Nuc-d, aes(x = probe, y = bTau)) +
...
fit_rhoa-Nuc-d <- lm(bTau ~ probe, data = data_rhoa-Nuc-d)

summary(fit_rhoa-Nuc-d)
